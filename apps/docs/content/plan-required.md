...
nothing. The mode bounds *when* a crew may act, never *what* it may do.
...
intent and proceeds. They compose: a crew can require a plan *and* gate the
...
chain. This is an off-chain supervision control that refuses to *start* work.
...
and how recently. Whether the plan actually *describes* the effect is a judgement
...
- **authored by the acting agent** — a seat cannot ride another seat's plan, and
  a *human* writing "we should ship this" is not the agent stating its intent.
- **in a thread that agent speaks in** — its own thread, a crew thread above it,
  ...
  nothing.
- **fresh** — inside the window (30 minutes by default), *or* tagged with the run
...
guarantee is the same — the operator saw *this run* say what it was about to do.
...
one has to refuse. Plan-required guards *legibility* — every onchain cap,
