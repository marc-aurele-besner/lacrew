...
/**
 * Contracts whose creation bytecode ships in @lacrew/core so the SDK can
 * deploy them at runtime. Only the per-node policy modules belong here: their
 * params are constructor-immutable, so "change a node's rate limit" means
 * "deploy a module with the new params and rebind" — which needs bytecode.
 */
const BYTECODE_CONTRACTS = [
  {
    file: "RateLimitPolicy.sol/RateLimitPolicy.json",
    exportName: "rateLimitPolicyBytecode",
  },
  {
    file: "TimeWindowPolicy.sol/TimeWindowPolicy.json",
    exportName: "timeWindowPolicyBytecode",
  },
  { file: "PolicyStack.sol/PolicyStack.json", exportName: "policyStackBytecode" },
];
...
function loadBytecode(relPath) {
  const full = join(contractsOut, relPath);
  const artifact = JSON.parse(readFileSync(full, "utf8"));
  const object = artifact.bytecode?.object;
  if (typeof object !== "string" || !object.startsWith("0x") || object.length <= 2) {
    throw new Error(`No creation bytecode in ${full}`);
  }
  return object;
}

function writeBytecodeTs(bytecodeByExport) {
  const parts = [
    "/**",
    " * Generated by packages/core/scripts/sync-abis.mjs — do not edit by hand.",
    " * Creation bytecode for the runtime-deployable policy modules.",
    " */",
    "",
  ];
  for (const [exportName, bytecode] of Object.entries(bytecodeByExport)) {
    parts.push(`export const ${exportName} = ${JSON.stringify(bytecode)} as const;`, "");
  }
  writeFileSync(join(coreRoot, "src/bytecode.generated.ts"), parts.join("\n"));
}
...
function main() {
  ...
  const bytecodes = {};
  for (const c of BYTECODE_CONTRACTS) {
    bytecodes[c.exportName] = loadBytecode(c.file);
  }
  writeBytecodeTs(bytecodes);
  console.log("Synced bytecode → packages/core/src/bytecode.generated.ts");
