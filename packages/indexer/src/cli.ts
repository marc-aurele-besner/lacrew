...
import { resolveIndexerPath } from "@lacrew/core/node";
import { EventWatcher } from "./watcher.js";

const rpcUrl = process.env.ANVIL_RPC ?? process.env.RPC_URL ?? "http://127.0.0.1:8545";
// Anchored to the workspace root, not this process's cwd — the orchestrator
// reads the same file from a different directory. See resolveWorkspacePath.
const storePath = resolveIndexerPath();
